<template>
  <div class="card-list grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
    <div v-if="cards.length === 0" class="no-cards col-span-full text-center text-gray-500">
      No cards available
    </div>
    <div
      v-else
    ...
    >
      <img :src="card.imageUrl" alt="Card Image" class="w-full h-48 object-contain bg-gray-100" />
      <div class="p-4 flex-1 flex flex-col">
        <h3 class="text-lg font-semibold mb-2">{{ card.name }}</h3>
        <!-- Quantity displayed only if showQuantity is true -->
        <p v-if="showQuantity" class="text-gray-600 mb-2">Quantity: {{ card.quantity }}</p>
        <!-- Display Price -->
