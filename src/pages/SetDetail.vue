        ...
        <div class="mt-auto flex items-center space-x-2">
          ...
          <button @click="toggleWishlist(card)" class="focus:outline-none">
            <component
              ...
              :class="wishlist[card.id] ? 'text-red-500' : 'text-gray-500 dark:text-gray-400'"
            />
          </button>
        </div>
        <!-- Confirmation Message -->
        <div v-if="successMessages[card.id]" class="mt-2 text-green-500 dark:text-green-400">
...
const toggleWishlist = async (card) => {
  ...
  if (wishlist[card.id]) {
    ...
    try {
      ...
      successMessages.value[card.id] = 'Kortet er fjernet fra din ønskeliste!'
    ...
    try {
      ...
      successMessages.value[card.id] = 'Kortet er tilføjet til din ønskeliste!'
