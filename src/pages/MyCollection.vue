<template>
  <div class="container mx-auto p-4">
    ...
    <div class="flex items-center justify-between mb-4">
      <h1 class="text-2xl font-bold text-gray-800 dark:text-white">My Collection</h1>
...
// Brug useSortable
const { sortedResults: sortedCollectionCards, handleSortChange } = useSortable(collectionCards)
...
const fetchCollection = async () => {
  ...
  try {
    const snapshot = await getDocs(collection(db, 'users', user.value.uid, 'collection'))
    const cards = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }))
    const updatedCards = await Promise.all(
      cards.map(async (card) => {
        const marketPrice = card.marketPrice || (await fetchMarketPrice(card.apiId)) || 0
        return {
          ...card,
          marketPrice,
...
const removeCardFromCollection = async (cardId) => {
  ...
  try {
    ...
    collectionCards.value = collectionCards.value.filter((card) => card.id !== cardId)
  } catch (error) {
...
const totalPrice = computed(() => {
  return collectionCards.value
    .reduce(
      (total, card) => total + (parseFloat(card.marketPrice) || 0) * (parseInt(card.quantity) || 0),
      0,
    )
    .toFixed(2)
})
