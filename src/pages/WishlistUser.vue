...
// Brug useSortable
const { sortedResults: sortedWishlistCards, handleSortChange } = useSortable(wishlistCards)
...
const fetchWishlist = async () => {
  ...
  try {
    const snapshot = await getDocs(collection(db, 'users', userId.value, 'wishlist'))
...
const removeCardFromWishlist = async (cardId) => {
  ...
  try {
    ...
    wishlistCards.value = wishlistCards.value.filter((card) => card.id !== cardId)
  } catch (error) {
