...
import { fetchApiDetails, fetchCardFromApi, fetchCardFromCollection } from '../services/cardService'
...
const fetchCardDetail = async () => {
  ...
  try {
    if (source === 'collection' || source === 'wishlist') {
      ...
      const collectionPath = source === 'collection' ? 'collection' : 'wishlist'
      card.value = await fetchCardFromCollection(userStore.user.uid, cardId, collectionPath)
